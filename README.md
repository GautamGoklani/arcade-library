...
What the exercise produced, though, is seven complete games in **strict MVP
WebAssembly**: four value types, one 64 KiB memory, structured control flow, and
two imports — `sinf` and `cosf` — which four of the seven do not need at all. No post-MVP feature, no feature detection, no fallback build. They
